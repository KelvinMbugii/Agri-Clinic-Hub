...
export default function AdminDashboard() {
  ...
  const pendingOfficers = useMemo(
    () => users.filter((u) => u.role === 'officer' && !u.isVerified),
    [users]
  );
    ...
    <div className="mx-auto max-w-6xl px-4 py-6 lg:flex lg:gap-6">
      ...
      <main className="min-w-0 flex-1">
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-3">
          <Link
            to="/admin/officers"
            className="rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-soft hover:bg-slate-50"
          >
            <div className="font-semibold text-slate-900">Review officers</div>
            <p className="mt-1 text-slate-600">
              {pendingOfficers.length} pending verification requests.
            </p>
          </Link>
          <Link
            to="/admin/articles"
          ...
          >
            ...
            <p className="mt-1 text-slate-600">
              Track prompts and disease detection behavior.
            </p>
          </Link>
          <Link
            to="/admin/settings"
            className="rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-soft hover:bg-slate-50"
          >
            <div className="font-semibold text-slate-900">Admin settings</div>
            <p className="mt-1 text-slate-600">
              Governance checklist and account overview.
            </p>
          </Link>
        </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Recent disease detection activity.
                </p>
              </div>
              <button
                onClick={refreshLogs}
