...
import { getArticlesRequest, deleteArticleRequest, createArticleRequest, updateArticleRequest } from "../../services/api"
...
export default function Articles() {
    ...
    const refreshArticles = async () => {       
        ...
        try {
            ...
            setError((s)  => ({...s, articles: err?.response?.data?.message || 'Failed to load articles'}))
        } finally {
    ...
    const saveArticle = async (e) => {
        ...
        try {
            ...
            setError((s) => ({ ...s, saving: err?.response?.data?.message || 'Failed to save article' }));
        } finally {
    ...
    const removeArticle = async (id) => {
        ...
        try {
            ...
            setError((s) => ({ ...s, saving: err?.response?.data?.message || 'Failed to delete article' }));
        }
    };
   ...
   return (
    <div className="mx-auto max-w-6xl px-4 py-6 lg:flex lg:gap-6">
      ...
      <main className="min-w-0 flex-1">
        <h1 className="text-2xl font-semibold text-slate-900">Articles & Guidance</h1>
        <p className="mt-1 text-sm text-slate-600">
          Publish agricultural advice for farmers.
        </p>

        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
            <h2 className="text-lg font-semibold">{editingId ? 'Edit Article' : 'Create Article'}</h2>
            <p className="mt-1 text-sm text-slate-600">Share practical guidance.</p>
